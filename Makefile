...
>	${ANSIBLE} \
		...
		--extra-vars '{"disk_id": "${DISK_ID}","encrypted_password":"$(value ENCRYPTED_ANSIBLE_USER_PASSWORD)","encryption_passphrase":"$(value ENCRYPTION_PASSPHRASE)","encrypt_disks":true}' \
		"localhost"
...
${PRODUCTION_K8S_APPS}: ANSIBLE_PLAYBOOK_OPTIONS := --inventory "production"\
							--tags "${ANSIBLE_TAGS}"\
							--extra-vars '{"network_configs_path":"network_configs.yml","talosconfig_path":"../prod-k8s-cluster/talosconfig","kubeconfig_path":"../prod-k8s-cluster/kubeconfig","k8s_configs_path":"k8s_configs.yml"}'
${PRODUCTION_K8S_APPS}:
