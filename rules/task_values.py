...
class TaskValuesRule(AnsibleLintRule):
    ...
    _ids = {
        ...
        "task-values[fqcn-in-filter]": "Use fully qualified collection names for ansible filters.",  # noqa E501
    ...
    def get_leaf_nodes(self, node: list[Any] | dict[Any, Any]) -> list[Any]:
        ...
        def extract_leaf_nodes(node: Any) -> list[Any]:
            ...
            for child in node.values() if isinstance(node, dict) else node:
                nodes += self.get_leaf_nodes(child)

            return nodes

        return extract_leaf_nodes(node)

    def get_filter_matcherrors(
